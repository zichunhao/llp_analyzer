...
{
  // OLD BRANCHES, NOT FROM NEW NANOAODS
  // fChain->SetBranchStatus("nMuons", 1);
  // fChain->SetBranchStatus("nMuon", 1);
  // fChain->SetBranchAddress("nMuon", &nMuons);
  // fChain->SetBranchAddress("Muon_pt", muonPt);
  ...
  fChain->SetBranchAddress("Muon_charge", muonCharge);
  fChain->SetBranchStatus("Muon_cleanmask", 1);
  fChain->SetBranchAddress("Muon_cleanmask", muonCleanmask);
  fChain->SetBranchStatus("Muon_dxy", 1);
  fChain->SetBranchAddress("Muon_dxy", muon_d0);
  fChain->SetBranchStatus("Muon_dxyErr", 1);
  fChain->SetBranchAddress("Muon_dxyErr", muon_d0Err);
  fChain->SetBranchStatus("Muon_dz", 1);
  ...
  fChain->SetBranchAddress("Muon_eta", muonEta);
  fChain->SetBranchStatus("Muon_fsrPhotonIdx", 1);
  fChain->SetBranchAddress("Muon_eta", muonEta);
  */
}
...
bool RazorAnalyzer::isPFTightJet(int i, bool applyTightLepVeto, string EraName)
{
  ...
  if (EraName == "Razor2016_07Aug2017Rereco")
  {
    ...
    {
      if (applyTightLepVeto)
        return (NHF < 0.90 && NEMF < 0.90 && NumConst > 1 && MUF < 0.8) && ((abs(jetEta[i]) <= 2.4 && CHF > 0 && CHM > 0 && CEMF < 0.90) || abs(jetEta[i]) > 2.4) && abs(jetEta[i]) <= 2.7;
      else
        return (NHF < 0.90 && NEMF < 0.90 && NumConst > 1) && ((abs(jetEta[i]) <= 2.4 && CHF > 0 && CHM > 0 && CEMF < 0.99) || abs(jetEta[i]) > 2.4) && abs(jetEta[i]) <= 2.7;
    }
  ...
  else if (EraName == "Razor2017_17Nov2017Rereco")
  {
    ...
    {
      if (applyTightLepVeto)
        return (NHF < 0.90 && NEMF < 0.90 && NumConst > 1 && MUF < 0.8) && ((abs(jetEta[i]) <= 2.4 && CHF > 0 && CHM > 0 && CEMF < 0.80) || abs(jetEta[i]) > 2.4) && abs(jetEta[i]) <= 2.7;
      else
        return (NHF < 0.90 && NEMF < 0.90 && NumConst > 1) && ((abs(jetEta[i]) <= 2.4 && CHF > 0 && CHM > 0) || abs(jetEta[i]) > 2.4) && abs(jetEta[i]) <= 2.7;
    }
    else if (abs(jetEta[i]) > 2.7 && abs(jetEta[i]) <= 3.0)
    {
  ...
  else if (EraName == "Razor2018_17SeptEarlyReReco")
  {
    ...
    {
      if (applyTightLepVeto)
        return (abs(jetEta[i]) <= 2.6 && CEMF < 0.8 && CHM > 0 && CHF > 0 && NumConst > 1 && NEMF < 0.9 && MUF < 0.8 && NHF < 0.9);
      else
        return (abs(jetEta[i]) <= 2.6 && CHM > 0 && CHF > 0 && NumConst > 1 && NEMF < 0.9 && NHF < 0.9);
    }
    else if (abs(jetEta[i]) > 2.6 && abs(jetEta[i]) <= 2.7)
    {
      if (applyTightLepVeto)
        return (abs(jetEta[i]) > 2.6 && abs(jetEta[i]) <= 2.7 && CEMF < 0.8 && CHM > 0 && NEMF < 0.99 && MUF < 0.8 && NHF < 0.9);
      else
        return (abs(jetEta[i]) > 2.6 && abs(jetEta[i]) <= 2.7 && CHM > 0 && NEMF < 0.99 && NHF < 0.9);
    }
    else if (abs(jetEta[i]) > 2.7 && abs(jetEta[i]) <= 3.0)
    {
...
bool RazorAnalyzer::isEGammaPOGVetoElectron(int i, bool applyID, bool applyIso, bool use25nsCuts, string EraName)
{
  ...
  if (applyID)
  {
    if (!passEGammaPOGVetoElectronID(i, use25nsCuts, EraName))
      pass = false;
  }
  if (applyIso)
  {
    if (!passEGammaPOGVetoElectronIso(i, use25nsCuts))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isEGammaPOGLooseElectron(int i, bool applyID, bool applyIso, bool use25nsCuts, string EraName)
{
  ...
  if (applyID)
  {
    if (!passEGammaPOGLooseElectronID(i, use25nsCuts, EraName))
      pass = false;
  }
  if (applyIso)
  {
    if (!passEGammaPOGLooseElectronIso(i, use25nsCuts))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isEGammaPOGMediumElectron(int i, bool applyID, bool applyIso, bool use25nsCuts, string EraName)
{
  ...
  if (applyID)
  {
    if (!passEGammaPOGMediumElectronID(i, use25nsCuts, EraName))
      pass = false;
  }
  if (applyIso)
  {
    if (!passEGammaPOGMediumElectronIso(i, use25nsCuts))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isEGammaPOGTightElectron(int i, bool applyID, bool applyIso, bool use25nsCuts, string EraName)
{
  ...
  if (applyID)
  {
    if (!passEGammaPOGTightElectronID(i, use25nsCuts, EraName))
      pass = false;
  }
  if (applyIso)
  {
    if (!passEGammaPOGTightElectronIso(i, use25nsCuts))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isLooseElectron(int i, bool applyID, bool applyIso, bool use25nsCuts, string EraName)
{
  ...
  if (applyIso)
  {
    if (!((ele_chargedMiniIso[i] + fmax(0.0, ele_photonAndNeutralHadronMiniIso[i] - fixedGridRhoFastjetAll * GetElectronEffectiveAreaMean(i) * pow(dr / 0.3, 2))) / elePt[i] < 0.1))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isMVANonTrigVetoElectron(int i, bool applyID, bool applyIso, string EraName)
{
  ...
  if (applyID)
  {
    if (!passMVANonTrigVetoElectronID(i, EraName))
      pass = false;
  }
  if (applyIso)
  {
    if (!passMVANonTrigVetoElectronIso(i))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::passEGammaPOGVetoElectronID(int i, bool use25nsCuts, string EraName)
{
  ...
  if (EraName == "Spring15")
  {
    ...
    {
      if (fabs(ele_dEta[i]) < 0.0152 && fabs(ele_dPhi[i]) < 0.216 && eleFull5x5SigmaIetaIeta[i] < 0.0114 && ele_HoverE[i] < 0.181 && fabs(ele_d0[i]) < 0.0564 && fabs(ele_dZ[i]) < 0.472 && fabs(ele_OneOverEminusOneOverP[i]) < 0.207 && ele_PassConvVeto[i] && ele_MissHits[i] <= 2)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.0113 && fabs(ele_dPhi[i]) < 0.237 && eleFull5x5SigmaIetaIeta[i] < 0.0352 && ele_HoverE[i] < 0.116 && fabs(ele_d0[i]) < 0.222 && fabs(ele_dZ[i]) < 0.921 && fabs(ele_OneOverEminusOneOverP[i]) < 0.174 && ele_PassConvVeto[i] && ele_MissHits[i] <= 3)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "Summer16")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00749 && fabs(ele_dPhi[i]) < 0.228 && eleFull5x5SigmaIetaIeta[i] < 0.0115 && ele_HoverE[i] < 0.356 && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.299 && ele_PassConvVeto[i] && ele_MissHits[i] <= 2)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00895 && fabs(ele_dPhi[i]) < 0.213 && eleFull5x5SigmaIetaIeta[i] < 0.037 && ele_HoverE[i] < 0.211 && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.15 && ele_PassConvVeto[i] && ele_MissHits[i] <= 3)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "2017_94X")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00463 && fabs(ele_dPhi[i]) < 0.148 && eleFull5x5SigmaIetaIeta[i] < 0.0126 && ele_HoverE[i] < 0.05 + 1.16 / eleE[i] + 0.0324 * fixedGridRhoFastjetAll / eleE[i] && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.209 && ele_PassConvVeto[i] && ele_MissHits[i] <= 2)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00814 && fabs(ele_dPhi[i]) < 0.19 && eleFull5x5SigmaIetaIeta[i] < 0.0457 && ele_HoverE[i] < 0.05 + 2.54 / eleE[i] + 0.183 * fixedGridRhoFastjetAll / eleE[i] && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.132 && ele_PassConvVeto[i] && ele_MissHits[i] <= 3)
      {
        pass = true;
      }
    }
...
bool RazorAnalyzer::passEGammaPOGLooseElectronID(int i, bool use25nsCuts, string EraName)
{
  ...
  if (EraName == "Spring15")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.0105 && fabs(ele_dPhi[i]) < 0.115 && eleFull5x5SigmaIetaIeta[i] < 0.0103 && ele_HoverE[i] < 0.104 && fabs(ele_d0[i]) < 0.0261 && fabs(ele_dZ[i]) < 0.41 && fabs(ele_OneOverEminusOneOverP[i]) < 0.102 && ele_PassConvVeto[i] && ele_MissHits[i] <= 2)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00814 && fabs(ele_dPhi[i]) < 0.182 && eleFull5x5SigmaIetaIeta[i] < 0.0301 && ele_HoverE[i] < 0.0897 && fabs(ele_d0[i]) < 0.118 && fabs(ele_dZ[i]) < 0.822 && fabs(ele_OneOverEminusOneOverP[i]) < 0.126 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "Summer16")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00477 && fabs(ele_dPhi[i]) < 0.222 && eleFull5x5SigmaIetaIeta[i] < 0.011 && ele_HoverE[i] < 0.298 && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.241 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00868 && fabs(ele_dPhi[i]) < 0.213 && eleFull5x5SigmaIetaIeta[i] < 0.0314 && ele_HoverE[i] < 0.101 && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.14 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "2017_94X")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00377 && fabs(ele_dPhi[i]) < 0.0884 && eleFull5x5SigmaIetaIeta[i] < 0.0112 && ele_HoverE[i] < 0.05 + 1.16 / eleE[i] + 0.0324 * fixedGridRhoFastjetAll / eleE[i] && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.193 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00674 && fabs(ele_dPhi[i]) < 0.169 && eleFull5x5SigmaIetaIeta[i] < 0.0425 && ele_HoverE[i] < 0.0441 + 2.54 / eleE[i] + 0.183 * fixedGridRhoFastjetAll / eleE[i] && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.111 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "vid")
  {
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (ele_passCutBasedIDLoose[i] && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10)
      {
        pass = true;
      }
    }
    ...
    {
      if (ele_passCutBasedIDLoose[i] && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2)
      {
        pass = true;
      }
    }
...
bool RazorAnalyzer::passEGammaPOGMediumElectronID(int i, bool use25nsCuts, string EraName)
{
  ...
  if (EraName == "Spring15")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.0103 && fabs(ele_dPhi[i]) < 0.0336 && eleFull5x5SigmaIetaIeta[i] < 0.0101 && ele_HoverE[i] < 0.0876 && fabs(ele_d0[i]) < 0.0118 && fabs(ele_dZ[i]) < 0.373 && fabs(ele_OneOverEminusOneOverP[i]) < 0.0174 && ele_PassConvVeto[i] && ele_MissHits[i] <= 2)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00733 && fabs(ele_dPhi[i]) < 0.114 && eleFull5x5SigmaIetaIeta[i] < 0.0283 && ele_HoverE[i] < 0.0678 && fabs(ele_d0[i]) < 0.0739 && fabs(ele_dZ[i]) < 0.602 && fabs(ele_OneOverEminusOneOverP[i]) < 0.0898 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "Summer16")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00311 && fabs(ele_dPhi[i]) < 0.103 && eleFull5x5SigmaIetaIeta[i] < 0.00998 && ele_HoverE[i] < 0.253 && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.134 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00609 && fabs(ele_dPhi[i]) < 0.045 && eleFull5x5SigmaIetaIeta[i] < 0.0298 && ele_HoverE[i] < 0.0878 && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.13 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "2017_94X")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.0032 && fabs(ele_dPhi[i]) < 0.0547 && eleFull5x5SigmaIetaIeta[i] < 0.0106 && ele_HoverE[i] < 0.046 + 1.16 / eleE[i] + 0.0324 * fixedGridRhoFastjetAll / eleE[i] && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.184 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00632 && fabs(ele_dPhi[i]) < 0.0394 && eleFull5x5SigmaIetaIeta[i] < 0.0387 && ele_HoverE[i] < 0.0275 + 2.52 / eleE[i] + 0.183 * fixedGridRhoFastjetAll / eleE[i] && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.0721 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
...
bool RazorAnalyzer::passEGammaPOGTightElectronID(int i, bool use25nsCuts, string EraName)
{
  ...
  if (EraName == "Spring15")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00926 && fabs(ele_dPhi[i]) < 0.0336 && eleFull5x5SigmaIetaIeta[i] < 0.0101 && ele_HoverE[i] < 0.0597 && fabs(ele_d0[i]) < 0.0111 && fabs(ele_dZ[i]) < 0.0466 && fabs(ele_OneOverEminusOneOverP[i]) < 0.012 && ele_PassConvVeto[i] && ele_MissHits[i] <= 2)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00724 && fabs(ele_dPhi[i]) < 0.0918 && eleFull5x5SigmaIetaIeta[i] < 0.0279 && ele_HoverE[i] < 0.0615 && fabs(ele_d0[i]) < 0.0351 && fabs(ele_dZ[i]) < 0.417 && fabs(ele_OneOverEminusOneOverP[i]) < 0.00999 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "Summer16")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00308 && fabs(ele_dPhi[i]) < 0.0816 && eleFull5x5SigmaIetaIeta[i] < 0.00998 && ele_HoverE[i] < 0.0414 && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.0129 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00605 && fabs(ele_dPhi[i]) < 0.0394 && eleFull5x5SigmaIetaIeta[i] < 0.0292 && ele_HoverE[i] < 0.0641 && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.0129 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "2017_94X")
  {
    ...
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (fabs(ele_dEta[i]) < 0.00255 && fabs(ele_dPhi[i]) < 0.022 && eleFull5x5SigmaIetaIeta[i] < 0.0104 && ele_HoverE[i] < 0.026 + 1.15 / eleE_SC[i] + 0.0324 * fixedGridRhoFastjetAll / eleE_SC[i] && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10 && fabs(ele_OneOverEminusOneOverP[i]) < 0.159 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
    ...
    {
      if (fabs(ele_dEta[i]) < 0.00501 && fabs(ele_dPhi[i]) < 0.0236 && eleFull5x5SigmaIetaIeta[i] < 0.0353 && ele_HoverE[i] < 0.0188 + 2.06 / eleE_SC[i] + 0.183 * fixedGridRhoFastjetAll / eleE_SC[i] && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2 && fabs(ele_OneOverEminusOneOverP[i]) < 0.0197 && ele_PassConvVeto[i] && ele_MissHits[i] <= 1)
      {
        pass = true;
      }
    }
  ...
  else if (EraName == "vid")
  {
    if (fabs(eleEta_SC[i]) < 1.479)
    {
      if (ele_passCutBasedIDTight[i] && fabs(ele_d0[i]) < 0.05 && fabs(ele_dZ[i]) < 0.10)
      {
        pass = true;
      }
    }
    ...
    {
      if (ele_passCutBasedIDTight[i] && fabs(ele_d0[i]) < 0.1 && fabs(ele_dZ[i]) < 0.2)
      {
        pass = true;
      }
    }
...
bool RazorAnalyzer::passMVANonTrigVetoElectronID(int i, string EraName)
{
  ...
  bool pass = false;
  if (passMVAVetoElectronID(i, EraName) && fabs(ele_ip3dSignificance[i]) < 4)
  {
    pass = true;
  }
  // cout << pass << "\n";
...
bool RazorAnalyzer::passMVAVetoElectronID(int i, string EraName)
{
  ...
  if (fabs(eleEta_SC[i]) < 0.8)
    ...
    subdet = 2;
  Int_t ptBin = 0;
  if (elePt[i] > 10.0 && elePt[i] <= 15.0)
    ptBin = 1;
  if (elePt[i] > 15.0 && elePt[i] <= 25.0)
    ptBin = 2;
  if (elePt[i] > 25.0)
    ptBin = 3;
  ...
  if (EraName == "17Nov2017Rereco")
  {
    ...
    if (subdet == 2 && ptBin == 0)
      MVACut = 0.176;
    // For pT 10-25
    if (subdet == 0 && (ptBin == 1 || ptBin == 2))
      MVACut = -0.788 - 0.00987 * (elePt[i] - 10);
    if (subdet == 1 && (ptBin == 1 || ptBin == 2))
      MVACut = -0.85 - 0.005 * (elePt[i] - 10);
    if (subdet == 2 && (ptBin == 1 || ptBin == 2))
      MVACut = -0.81 - 0.00513 * (elePt[i] - 10);
    // For pT>25
    if (subdet == 0 && ptBin == 3)
      MVACut = -0.64;
    if (subdet == 1 && ptBin == 3)
  ...
  else
  {
    ...
    // For pT 15-25
    if (subdet == 0 && ptBin == 2)
      MVACut = -0.48 - 0.037 * (elePt[i] - 15);
    if (subdet == 1 && ptBin == 2)
      MVACut = -0.67 - 0.024 * (elePt[i] - 15);
    if (subdet == 2 && ptBin == 2)
      MVACut = -0.49 - 0.034 * (elePt[i] - 15);
    // For pT>25
    if (subdet == 0 && ptBin == 3)
      MVACut = -0.85;
    if (subdet == 1 && ptBin == 3)
...
bool RazorAnalyzer::passMVALooseElectronID(int i, string EraName)
{
  ...
  if (EraName == "17Nov2017Rereco")
  {
    ...
    // For pT 15-25
    if (subdet == 0 && ptBin == 2)
      MVACut = 0.66 - 0.064 * (elePt[i] - 15);
    if (subdet == 1 && ptBin == 2)
      MVACut = 0.44 - 0.057 * (elePt[i] - 15);
    if (subdet == 2 && ptBin == 2)
      MVACut = 0.49 - 0.030 * (elePt[i] - 15);
    // For pT>25
    if (subdet == 0 && ptBin == 3)
      MVACut = 0.02;
    if (subdet == 1 && ptBin == 3)
...
bool RazorAnalyzer::passHZZElectronPreselection(int i)
{
  ...
  bool pass = false;
  if (ele_IDMVAHZZ[i] > MVACut && fabs(ele_d0[i]) < 0.5 && fabs(ele_dZ[i]) < 1.0)
  {
    pass = true;
  }
...
bool RazorAnalyzer::isHZZElectron(int i)
{
  ...
  bool pass = false;
  if (ele_IDMVAHZZ[i] > MVACut && fabs(ele_d0[i]) < 0.5 && fabs(ele_dZ[i]) < 1.0 && fabs(ele_d0[i]) < 0.05 && ((ele_chargedIso[i] + fmax(0.0, ele_photonIso[i] + ele_neutralHadIso[i] - 0.5 * ele_pileupIso[i])) / elePt[i] < 0.4))
  {
    pass = true;
  }
...
bool RazorAnalyzer::matchTagElectronHLTFilters(int i)
{
  ...
  if (
      // Data filters
      ele_passHLTFilter[i][1] || ele_passHLTFilter[i][5] || ele_passHLTFilter[i][6] || ele_passHLTFilter[i][12] || ele_passHLTFilter[i][13] || ele_passHLTFilter[i][49] || ele_passHLTFilter[i][53] || ele_passHLTFilter[i][57] || ele_passHLTFilter[i][60]
      // MC filters
      || ele_passHLTFilter[i][64] || ele_passHLTFilter[i][3] || ele_passHLTFilter[i][8] || ele_passHLTFilter[i][10] || ele_passHLTFilter[i][15])
  {
    match = true;
  }
...
bool RazorAnalyzer::matchProbeElectronHLTFilters(int i)
{
  bool match = false;
  if (
      ele_passHLTFilter[i][50] || ele_passHLTFilter[i][51] || ele_passHLTFilter[i][61] || ele_passHLTFilter[i][62])
  {
    match = true;
  }
...
bool RazorAnalyzer::matchProbeSCHLTFilters(int i)
{
  bool match = false;
  if (
      ele_passHLTFilter[i][54] || ele_passHLTFilter[i][55] || ele_passHLTFilter[i][58] || ele_passHLTFilter[i][59])
  {
    match = true;
  }
...
{
  if (analysisTag == "2015")
    return matchElectronHLTFilters2015(i, HLTFilter);
  else if (analysisTag == "2016")
    return matchElectronHLTFilters2016(i, HLTFilter);
  else
  {
...
bool RazorAnalyzer::matchElectronHLTFilters2016(int i, string HLTFilter)
{
  ...
  if (HLTFilter == "Ele105")
  {
    if (ele_passHLTFilter[i][17])
      match = true;
  }
  if (HLTFilter == "Ele115")
  {
    if (ele_passHLTFilter[i][19])
      match = true;
  }
...
bool RazorAnalyzer::isMuonPOGLooseMuon(int i, bool applyID, bool applyIso)
{
  ...
  if (applyIso)
  {
    if (!((muon_chargedIso[i] + fmax(0.0, muon_photonIso[i] + muon_neutralHadIso[i] - 0.5 * muon_pileupIso[i])) / muonPt[i] < 0.25))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isMuonPOGTightMuon(int i, bool applyID, bool applyIso)
{
  ...
  if (applyIso)
  {
    if (!((muon_chargedIso[i] + fmax(0.0, muon_photonIso[i] + muon_neutralHadIso[i] - 0.5 * muon_pileupIso[i])) / muonPt[i] < 0.15))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isVetoMuon(int i, bool applyID, bool applyIso)
{
  ...
  if (applyID)
  {
    if (!(muonIsLoose[i] && fabs(muon_ip3dSignificance[i]) < 4))
      pass = false;
  }
  if (applyIso)
  {
...
bool RazorAnalyzer::isLooseMuon(int i, bool applyID, bool applyIso)
{
  ...
  if (applyIso)
  {
    if (!((muon_chargedMiniIso[i] + fmax(0.0, muon_photonAndNeutralHadronMiniIso[i] - fixedGridRhoFastjetAll * GetMuonEffectiveAreaMean(i, "neutral") * pow(dr / 0.3, 2))) / muonPt[i] < 0.2))
      pass = false;
  }
  return pass;
}
...
bool RazorAnalyzer::isTightMuon(int i, bool applyID, bool applyIso)
{
  ...
  if (applyID)
  {
    if (!(muonIsMedium[i] && fabs(muon_ip3dSignificance[i]) < 4 && fabs(muon_d0[i]) < 0.2))
      pass = false;
  }
  if (applyIso)
  {
...
{
  bool pass = false;
  if (muonIsLoose[i] && fabs(muon_d0[i]) < 0.5 && fabs(muon_dZ[i]) < 1.0)
  {
    pass = true;
  }
  return pass;
...
{
  bool pass = false;
  if (muonIsLoose[i] && fabs(muon_d0[i]) < 0.5 && fabs(muon_dZ[i]) < 1.0 && fabs(muon_ip3dSignificance[i]) < 4 && (((muon_chargedIso[i] + fmax(0.0, muon_photonIso[i] + muon_neutralHadIso[i] - 0.5 * muon_pileupIso[i])) / muonPt[i] < 0.4)))
  {
    pass = true;
  }
  return pass;
...
bool RazorAnalyzer::matchMuonHLTFilters(int i, string HLTFilter)
{
  ...
  {
    if (muon_passHLTFilter[i][3])
      match = true;
  }
  if (HLTFilter == "IsoMu22")
  {
    if (muon_passHLTFilter[i][7] || muon_passHLTFilter[i][9])
      match = true;
  }
  if (HLTFilter == "IsoMu24")
  {
    if (muon_passHLTFilter[i][11] || muon_passHLTFilter[i][12])
      match = true;
  }
  if (HLTFilter == "IsoTkMu20")
  {
    if (muon_passHLTFilter[i][16])
      match = true;
  }
  if (HLTFilter == "IsoTkMu22")
  {
    if (muon_passHLTFilter[i][17] || muon_passHLTFilter[i][18])
      match = true;
  }
  if (HLTFilter == "IsoTkMu24")
  {
    if (muon_passHLTFilter[i][19])
      match = true;
  }
  if (HLTFilter == "IsoMu27")
  {
    if (muon_passHLTFilter[i][14])
      match = true;
  }
  if (HLTFilter == "IsoTkMu27")
  {
    if (muon_passHLTFilter[i][21])
      match = true;
  }
  if (HLTFilter == "Mu50")
  {
    if (muon_passHLTFilter[i][22])
      match = true;
  }
  if (HLTFilter == "Mu55")
  {
    if (muon_passHLTFilter[i][25])
      match = true;
  }
  if (HLTFilter == "Mu45_eta2p1")
  {
    if (muon_passHLTFilter[i][26])
      match = true;
  }
  if (HLTFilter == "TkMu50")
  {
    if (muon_passHLTFilter[i][23])
      match = true;
  }
                                                ...
                                                bool printDebug)
{
  ...
  if (printDebug)
    cout << "Computing Jet Energy Corrections for jet with raw momentum: " << jetRawPt << " " << jetEta << " " << jetPhi << "\n";
  ...
  for (UInt_t j = 0; j < corrections.size(); ++j)
  {
    ...
    if (jetCorrectionLevel >= 0 && int(j) > jetCorrectionLevel)
      continue;

    double currentCorrection = corrections.at(j) / cumulativeCorrection;
    cumulativeCorrection = corrections.at(j);
    if (printDebug)
      cout << "Correction Level " << j << " : current correction = " << currentCorrection << " , cumulative correction = " << cumulativeCorrection << "\n";
  }
  if (printDebug)
    cout << "Final Cumulative Correction: " << cumulativeCorrection << "\n";
...
double RazorAnalyzer::JetEnergySmearingFactor(double jetPt, double jetEta, double NPU, SimpleJetResolution *JetResolutionCalculator, TRandom3 *random)
{
  ...
  if (fabs(jetEta) < 0.5)
    c = 1.079;
  else if (fabs(jetEta) < 1.1)
    c = 1.099;
  else if (fabs(jetEta) < 1.7)
    c = 1.121;
  else if (fabs(jetEta) < 2.3)
    c = 1.208;
  else if (fabs(jetEta) < 2.8)
    c = 1.254;
  else if (fabs(jetEta) < 3.2)
    c = 1.395;
  else if (fabs(jetEta) < 5.0)
    c = 1.056;
...
double RazorAnalyzer::BTagScaleFactor(double jetPt, bool CSVM, string option)
{
  ...
  if (option == "up" || option == "down")
  {
    ...
    if (jetPt < 30)
      uncertainty = 0.0415707;
    else if (jetPt < 40)
      uncertainty = 0.0204209;
    else if (jetPt < 50)
      uncertainty = 0.0223227;
    else if (jetPt < 60)
      uncertainty = 0.0206655;
    else if (jetPt < 70)
      uncertainty = 0.0199325;
    else if (jetPt < 80)
      uncertainty = 0.0174121;
    else if (jetPt < 100)
      uncertainty = 0.0202332;
    else if (jetPt < 120)
      uncertainty = 0.0182446;
    else if (jetPt < 160)
      uncertainty = 0.0159777;
    else if (jetPt < 210)
      uncertainty = 0.0218531;
    else if (jetPt < 260)
      uncertainty = 0.0204688;
    else if (jetPt < 320)
      uncertainty = 0.0265191;
    else if (jetPt < 400)
      uncertainty = 0.0313175;
    else if (jetPt < 500)
      uncertainty = 0.0415417;
    else if (jetPt < 600)
      uncertainty = 0.0740446;
    else if (jetPt < 800)
      uncertainty = 0.0596716;
    else
      uncertainty = 2 * 0.0596716;

    if (option == "up")
      tmpCorrFactor += uncertainty;
    else if (option == "down")
      tmpCorrFactor -= uncertainty;
  }
...
bool RazorAnalyzer::photonPassesIsolation(int i, double PFChHadIsoCut, double PFNeuHadIsoCut, double PFPhotIsoCut, bool useEffectiveArea90, bool usePrivatePF, bool usePFClusterIso)
{
  ...
  if (usePrivatePF)
    PFIsoCorrected_ChHad = fmax(pho_sumChargedHadronPt[i] - fixedGridRhoFastjetAll * effAreaChargedHadrons, 0.);
  if (usePFClusterIso)
    PFIsoCorrected_ChHad = fmax(pho_trkSumPtHollowConeDR03[i] - fixedGridRhoFastjetAll * effAreaChargedHadrons_ClusterIso, 0.);
  if (PFIsoCorrected_ChHad > PFChHadIsoCut)
    return false;
  ...
  if (usePrivatePF)
    PFIsoCorrected_NeuHad = fmax(pho_sumNeutralHadronEt[i] - fixedGridRhoFastjetAll * effAreaNeutralHadrons, 0.);
  if (usePFClusterIso)
    PFIsoCorrected_NeuHad = fmax(pho_sumNeutralHadronEt[i] - fixedGridRhoFastjetAll * effAreaNeutralHadrons, 0.);
  if (PFIsoCorrected_NeuHad > PFNeuHadIsoCut)
    return false;
  ...
  if (usePrivatePF)
    PFIsoCorrected_Photons = fmax(pho_sumPhotonEt[i] - fixedGridRhoFastjetAll * effAreaPhotons, 0.);
  if (usePFClusterIso)
    PFIsoCorrected_Photons = fmax(pho_ecalPFClusterIso[i] - fixedGridRhoFastjetAll * effAreaPhotons_ClusterIso, 0.);
  if (PFIsoCorrected_Photons > PFPhotIsoCut)
    return false;
...
bool RazorAnalyzer::photonPassLooseIDWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    ...
    {
      if (pho_HoverE[i] > 0.0597)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.01031)
        pass = false;
    }
    else
    {
      if (pho_HoverE[i] > 0.0481)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.03013)
        pass = false;
    }
  }
  else
...
bool RazorAnalyzer::photonPassMediumIDWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    if (fabs(pho_superClusterEta[i]) < 1.479)
    {
      if (pho_HoverE[i] > 0.0396)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.01022)
        pass = false;
    }
    else
    {
      if (pho_HoverE[i] > 0.0219)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.03001)
        pass = false;
    }
  }
  else
...
bool RazorAnalyzer::photonPassTightIDWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    if (fabs(pho_superClusterEta[i]) < 1.479)
    {
      if (pho_HoverE[i] > 0.0269)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.00994)
        pass = false;
    }
    else
    {
      if (pho_HoverE[i] > 0.0213)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.03000)
        pass = false;
    }
  }
  else
...
bool RazorAnalyzer::photonPassLooseID(int i, bool use25nsCuts)
{
  ...
  if (!photonPassLooseIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassMediumID(int i, bool use25nsCuts)
{
  ...
  if (!photonPassMediumIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassTightID(int i, bool use25nsCuts)
{
  ...
  if (!photonPassTightIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassLooseIso(int i, bool use25nsCuts, bool usePrivatePF, bool usePFClusterIso)
{
  ...
  if (use25nsCuts)
  {
    ...
    {
      if (!usePFClusterIso)
        return photonPassesIsolation(i, 1.295, 10.910 + 0.0148 * phoPt[i] + 0.000017 * phoPt[i] * phoPt[i], 3.630 + 0.0047 * phoPt[i], true, usePrivatePF, usePFClusterIso);
      else
        return photonPassesIsolation(i, 8.5 + 0.00091 * phoPt[i], 10.910 + 0.0148 * phoPt[i] + 0.000017 * phoPt[i] * phoPt[i], 8.0 + 0.00092 * phoPt[i], true, usePrivatePF, usePFClusterIso);
    }
    else
    {
  ...
  if (use25nsCuts)
  {
    if (fabs(pho_superClusterEta[i]) < 1.479)
    {
      if (!usePFClusterIso)
        ...
        return photonPassesIsolation(i, 5.5 + 0.00094 * phoPt[i], 0.264 + 0.0148 * phoPt[i] + 0.000017 * phoPt[i] * phoPt[i], 5.0 + 0.00092 * phoPt[i], true, usePrivatePF, true);
    }
    else
    {
...
bool RazorAnalyzer::isLoosePhoton(int i, bool use25nsCuts)
{
  ...
  bool pass = true;
  if (!isLoosePhotonWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::isMediumPhoton(int i, bool use25nsCuts)
{
  ...
  if (!isMediumPhotonWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
{
  bool pass = true;
  if (!isTightPhotonWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::isLoosePhotonWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (!photonPassLooseIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassLooseIso(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isMediumPhotonWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (!photonPassMediumIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassMediumIso(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isTightPhotonWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (!photonPassTightIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassTightIso(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isLooseDelayedPhotonWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (!photonPassLooseDelayedIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassLooseIso(i, use25nsCuts, false, true))
    pass = false;
...
bool RazorAnalyzer::isMediumDelayedPhotonWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (!photonPassMediumDelayedIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassMediumIso(i, use25nsCuts, false))
    pass = false;
...
bool RazorAnalyzer::isTightDelayedPhotonWithoutEleVeto(int i, bool use25nsCuts)
{
  ...
  if (!photonPassTightDelayedIDWithoutEleVeto(i, use25nsCuts))
    pass = false;
  if (!photonPassTightIso(i, use25nsCuts, false, true))
    pass = false;
...
bool RazorAnalyzer::photonPassLooseIDWithoutEleVetoExo15004(int i)
{
  ...
  {
    if (pho_HoverE[i] > 0.05)
      pass = false;
    if (phoFull5x5SigmaIetaIeta[i] > 0.0105)
      pass = false;
  }
  else
  {
    if (pho_HoverE[i] > 0.05)
      pass = false;
    if (phoFull5x5SigmaIetaIeta[i] > 0.0280)
      pass = false;
  }
  return pass;
};
...
bool RazorAnalyzer::photonPassLooseIDWithoutEleVeto_2017(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    if (fabs(pho_superClusterEta[i]) < 1.479)
    {
      if (pho_HoverE[i] > 0.105)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.0103)
        pass = false;
    }
    else
    {
      if (pho_HoverE[i] > 0.029)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.0276)
        pass = false;
    }
  }
  else
...
bool RazorAnalyzer::photonPassMediumIDWithoutEleVeto_2017(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    ...
    {
      if (pho_HoverE[i] > 0.027)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.0271)
        pass = false;
    }
  }
  else
...
bool RazorAnalyzer::photonPassLooseID_2017(int i, bool use25nsCuts)
{
  ...
  if (!photonPassLooseIDWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassMediumID_2017(int i, bool use25nsCuts)
{
  ...
  if (!photonPassMediumIDWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassTightID_2017(int i, bool use25nsCuts)
{
  ...
  if (!photonPassTightIDWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::isLoosePhotonWithoutEleVeto_2017(int i, bool use25nsCuts)
{
  ...
  if (!photonPassLooseIDWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassLooseIso_2017(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isMediumPhotonWithoutEleVeto_2017(int i, bool use25nsCuts)
{
  ...
  if (!photonPassMediumIDWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassMediumIso_2017(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isTightPhotonWithoutEleVeto_2017(int i, bool use25nsCuts)
{
  ...
  if (!photonPassTightIDWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassTightIso_2017(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isMediumPhoton_2017(int i, bool use25nsCuts)
{
  ...
  if (!isMediumPhotonWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
{
  bool pass = true;
  if (!isTightPhotonWithoutEleVeto_2017(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassLooseIDWithoutEleVeto_94X(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    if (fabs(pho_superClusterEta[i]) < 1.479)
    {
      if (pho_HoverE[i] > 0.04596)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.0106)
        pass = false;
    }
    else
    {
      if (pho_HoverE[i] > 0.0590)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.0272)
        pass = false;
    }
  }
  else
...
bool RazorAnalyzer::photonPassMediumIDWithoutEleVeto_94X(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    if (fabs(pho_superClusterEta[i]) < 1.479)
    {
      if (pho_HoverE[i] > 0.02197)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.01015)
        pass = false;
    }
    else
    {
      if (pho_HoverE[i] > 0.0326)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.0272)
        pass = false;
    }
...
bool RazorAnalyzer::photonPassTightIDWithoutEleVeto_94X(int i, bool use25nsCuts)
{
  ...
  if (use25nsCuts)
  {
    if (fabs(pho_superClusterEta[i]) < 1.479)
    {
      if (pho_HoverE[i] > 0.02148)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.00996)
        pass = false;
    }
    else
    {
      if (pho_HoverE[i] > 0.0321)
        pass = false;
      if (phoFull5x5SigmaIetaIeta[i] > 0.0271)
        pass = false;
    }
...
bool RazorAnalyzer::photonPassLooseID_94X(int i, bool use25nsCuts)
{
  ...
  if (!photonPassLooseIDWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassMediumID_94X(int i, bool use25nsCuts)
{
  ...
  if (!photonPassMediumIDWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::photonPassTightID_94X(int i, bool use25nsCuts)
{
  ...
  if (!photonPassTightIDWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
bool RazorAnalyzer::isLoosePhotonWithoutEleVeto_94X(int i, bool use25nsCuts)
{
  ...
  if (!photonPassLooseIDWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassLooseIso_94X(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isMediumPhotonWithoutEleVeto_94X(int i, bool use25nsCuts)
{
  ...
  if (!photonPassMediumIDWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassMediumIso_94X(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isTightPhotonWithoutEleVeto_94X(int i, bool use25nsCuts)
{
  ...
  if (!photonPassTightIDWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassTightIso_94X(i, use25nsCuts))
    pass = false;
...
bool RazorAnalyzer::isMediumPhoton_94X(int i, bool use25nsCuts)
{
  ...
  if (!isMediumPhotonWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
{
  bool pass = true;
  if (!isTightPhotonWithoutEleVeto_94X(i, use25nsCuts))
    pass = false;
  if (!photonPassesElectronVeto(i))
    pass = false;
...
int RazorAnalyzer::findClosestGenElectron(double eta, double phi)
{
  ...
  for (int j = 0; j < nGenParticle; j++)
  {
    ...
    if (abs(gParticleId[j]) != 11)
      continue;
    if (deltaR(eta, phi, gParticleEta[j], gParticlePhi[j]) < 0.1 && deltaR(eta, phi, gParticleEta[j], gParticlePhi[j]) < minDR)
    {
      matchedIndex = j;
...
int RazorAnalyzer::findClosestGenJet(double eta, double phi)
{
  ...
  for (int j = 0; j < nGenJets; j++)
  {
    ...
    if (deltaR(eta, phi, genJetEta[j], genJetPhi[j]) < 0.3 && deltaR(eta, phi, genJetEta[j], genJetPhi[j]) < minDR)
    {
      matchedIndex = j;
...
{
  if (abs(gParticleId[index]) == 15 && gParticleStatus[index] == 2 && (abs(gParticleMotherId[index]) == 24 || abs(gParticleMotherId[index]) == 23))
  {
...
int RazorAnalyzer::findClosestRecoTau(double eta, double phi)
{
  ...
  {
    if (deltaR(eta, phi, tauEta[j], tauPhi[j]) < 0.1 && deltaR(eta, phi, tauEta[j], tauPhi[j]) < minDR)
    {
      matchedIndex = j;
...
int RazorAnalyzer::GetTauMatchedID(double eta, double phi)
{
  ...
  if (matchedIndex < 0)
    matchedIndex = findClosestGenMuon(eta, phi);
  if (matchedIndex >= 0)
  {
  ...
  if (matchedIndex < 0)
    matchedIndex = findClosestGenElectron(eta, phi);
  if (matchedIndex >= 0)
  {
    return gParticleId[matchedIndex];
  }
...
int RazorAnalyzer::findClosestParton(float eta, float phi)
{
  ...
  for (int j = 0; j < nGenParticle; j++)
  {
    ...
    if (!(((abs(gParticleId[j]) >= 1 && abs(gParticleId[j]) <= 5) || abs(gParticleId[j]) == 21) && gParticleStatus[j] == 23))
      continue;
...
bool RazorAnalyzer::isHadronicDecay(int index, int status)
{
  for (int j = 0; j < nGenParticle; j++)
  {
    ...
    {
      if (abs(gParticleId[j]) > 0 && abs(gParticleId[j]) < 5)
        return true;
    }
  }
  return false;
                                                       ...
                                                       int id, int status, double r)
{
  ...
  for (int j = 0; j < nGenParticle; j++)
  {
    if (abs(gParticleId[j]) != abs(id))
      continue;
    if (gParticleStatus[j] != status)
      continue;
    ...
    if (thisJet.Pt() > 30 && fabs(thisJet.Eta()) < 2.4 && jetPassIDLoose[i])
    {
      ...
      for (int g = 0; g < nGenParticle; g++)
      {
        ...
        if (gParticleStatus[g] == 23 && abs(gParticleId[g]) <= 5 &&
            (abs(gParticleMotherId[g]) == 6 || abs(gParticleMotherId[g]) == 23 || abs(gParticleMotherId[g]) == 24 || abs(gParticleMotherId[g]) == 25 || abs(gParticleMotherId[g]) > 1e6))
        {
          match = true;
        }
      }
...
bool RazorAnalyzer::matchesVetoLepton(float eta, float phi, float dR)
{
  ...
  {
    if (muonPt[i] < 5 || fabs(muonEta[i]) > 2.4)
      continue;
  ...
  {
    if (elePt[i] < 5 || fabs(eleEta[i]) > 2.5)
      continue;
...
vector<TLorentzVector> RazorAnalyzer::getHemispheres(vector<TLorentzVector> jets)
{
  ...
  if (nJets < 2)
  { // return empty hemispheres if there are fewer than 2 jets provided
...
int RazorAnalyzer::SubtractParticleFromCollection(TLorentzVector ToSubtract, vector<TLorentzVector> &Collection, float deltaRMatch)
{
  ...
  if (closestDR < 0 || closestDR > deltaRMatch)
  { // if we didn't look at any objects or we didn't get one within 0.4, return -1
    return -1;
  }
...
double RazorAnalyzer::calcMT2(float testMass, bool massive, vector<TLorentzVector> jets, TLorentzVector MET, int hemi_seed, int hemi_association)
{
  ...
  if (jets.size() < 2)
    return -9999; // need at least two jets for the calculation
